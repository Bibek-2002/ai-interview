...
export class OpenAIService extends EventEmitter {
  ...
  async generateAnswer(question: string): Promise<string> {
    ...
    // A short context is enough for follow-up questions and reduces prompt
    // size, which improves Gemini's time-to-first-token.
    this.history = this.history.slice(-6)
    const answer = await this.request({
      systemInstruction: {
        parts: [
          {
            text: `You are answering a live interview as the candidate. Be natural, direct, and concise. Usually use 2-4 sentences; never use generic AI introductions. Candidate background: ${this.config.resumeDescription || 'Not supplied.'}`
          }
        ]
      },
      contents: this.history,
      generationConfig: {
        // Keep enough room for a complete answer to compound interview
        // questions; a too-small cap makes Gemini finish with MAX_TOKENS.
        maxOutputTokens: this.config.maxTokens || 700,
  ...
  async generateSolutionFromImage(
    ...
    console.log('[Gemini] Generating solution from image, type:', questionType)
    const data = imageBase64.includes(',') ? imageBase64.split(',')[1] : imageBase64
    return this.request({
      systemInstruction: {
        parts: [
          {
            text: 'Solve the interview problem clearly with explanation, code, complexity, and edge cases.'
          }
        ]
      },
      contents: [
        {
          role: 'user',
          parts: [
            { text: questionText || 'Extract the question from this screenshot and solve it.' },
  ...
  private async request(payload: Record<string, unknown>): Promise<string> {
    ...
    console.log('[Gemini] Complete answer:\n' + answer)
